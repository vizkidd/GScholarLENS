...
function processAuthorPositions(yearwiseData, author, author_list, citationCount, index, qscore, year, publication) {
  ...
  // let position = false; //return false if author not found  
...
self.onmessage = async ({ data }) => {
  ...
  try {
    for (const publication of batch) {
      ...
      let author_pos_string = "NA";
      let author_list = authors.split(',').map(author => author.trim());
      publication.total_authors = author_list.length; //authors.includes("...") ? author_list.length - 1 : author_list.length;
      ...
      if (task === 'initialScrape') {
        ...
        // console.warn("initialScrape:",pub_idx, publication.title, publication.authors); // DEBUG
        if ((uniq(author_list_filtered).length > 1 || author_list_filtered.length === 0) && authors.includes("...")) {
          ...
          authorFound = false;
          publication.extended_scrape = true;
          ...
          // console.log("INITIAL WORKER: ", publication, author_list); //DEBUG
          sendZeroCopyArr(initZeroCopyArr);
          continue;
        }
        ...
        author_list.forEach(async (author, i) => {
          ...
          if (author.includes("...") && !authorFound) {
            // publication.authors = "Pending";
            publication.extended_scrape = true;
            publication.considered = false;
            // await new Promise(r => setTimeout(r, 0));  // Allow other tasks to run
          ...
          if (non_specialAuthor_name.includes(authorNameShort.toLowerCase()) || non_specialAuthor_name.includes(authorNameLong.toLowerCase()) || areEqual(author, authorName) || otherNamesList.includes(normalizeString(author.trim()))) {
            ...
            if (regexInitial.some(x => x === true)) {
              ...
              // }
              author_pos_string = processAuthorPositions(yearwiseData, author, author_list, citationCount, i, publication.journalRanking, publication.year, pub_idx, publication);
              authorFound = true;
              // console.log("INITIAL WORKER (FOUND) :", pub_idx, publication.title, author_pos_string); // DEBUG
            }
          }
          // await new Promise(r => setTimeout(r, 0));  // Allow other tasks to run
        });
        ...
        if (!authorFound) {
          ...
          publication.author_pos = author_pos_string;
        ...
        // console.log("EXTENDED WORKER: ", publication, author_list); //DEBUG
        sendZeroCopyArr(extZeroCopyArr);
      }
    }

    function serializeValue(v) {
      if (v instanceof Map) {
        return Array.from(v.entries()).map(([k, sub]) => [k, serializeValue(sub)]);
      }
      if (Array.isArray(v)) {
        return v.slice();
      }
    ...
    // Now serialize your whole yearwiseData:
    const finalZeroCopyArr = {
      type: 'done',
      task,
      authorNamesConsidered,
      yearwiseData: Array.from(yearwiseData.entries()).map(
        ([year, innerMap]) => [
          year,
          serializeValue(innerMap)  // innerMap is itself a Map
        ]
      )
    };
    // console.log("WORKER yearwiseData: ", yearwiseData); //DEBUG
    sendZeroCopyArr(finalZeroCopyArr);
    //   self.postMessage({ type: 'done' , task: task, authorNamesConsidered: authorNamesConsidered }, [finalBytes.buffer]);

  } catch (err) {
    const errZeroCopyArr = {
      type: 'error', task: task, error: err.message
    };
    const errJson = JSON.stringify(errZeroCopyArr);
    const errBytes = encoder.encode(errJson);            // Uint8Array
    self.postMessage(errBytes.buffer, [errBytes.buffer]);
  }
};
