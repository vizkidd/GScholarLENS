...
self.onmessage = async ({ data }) => {
    ...
    try {
      for (const publication of batch) {
        ...
        self.postMessage(initBytes.buffer, [initBytes.buffer]);
          ...
          const extZeroCopyArr =  {
            ...
            // 1) Turn to JSON
            const extJson = JSON.stringify(extZeroCopyArr);
            // 2) Encode as UTF-8 bytes in a Uint8Array
            const extBytes   = encoder.encode(extJson);            // Uint8Array
        //   self.postMessage({ type: 'working' , task: 'extendedScrape', authorFound: authorFound, extended_check: extended_check, publication: publication }); //authorRegexes: authorRegexes, authorRegexesEx: authorRegexesEx
        self.postMessage(extBytes.buffer, [extBytes.buffer]); //authorRegexes: authorRegexes, authorRegexesEx: authorRegexesEx
      ...
      const finalZeroCopyArr =  {
        type: 'done' ,
        task: task, 
        authorNamesConsidered: authorNamesConsidered,
        yearwiseData: Array.from(yearwiseData.entries()).map(
          ([year, innerMap]) => [
            year,
            Array.from(innerMap.entries()).map(([ key, value ]) => [
              key,
    ...
    //   console.log("yearwiseData: ", yearwiseData); //DEBUG
      // 1) Turn to JSON
        const finalJson = JSON.stringify(finalZeroCopyArr);
        // 2) Encode as UTF-8 bytes in a Uint8Array
        const finalBytes   = encoder.encode(finalJson);            // Uint8Array

    //   self.postMessage({ type: 'done' , task: task, authorNamesConsidered: authorNamesConsidered }, [finalBytes.buffer]);
    self.postMessage( finalBytes.buffer, [finalBytes.buffer]);
    } catch (err) {
