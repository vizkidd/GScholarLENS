...
`; const Publications = [[], [], [], []]; function generatePublications() {
    let t = ["First Author", "Second Author", "Co-Author", "Corresponding Author"], o = [.9, .5, .1, 1]; for (let r = 0; r < MAX_PUBLICATIONS; r++) {
        let e = getUniqueRandomPair(AuthorCitationDistributions, selectedList), a = e[0], n = getRandomNumber(a, 10), i = e[1]; selectedList.push(e); let d = AuthorCitationDistributions[a][i], $ = t[a], s = 2 === a ? n > 6 ? .1 : .25 : o[a], l = { id: r, title: crypto.randomUUID().replaceAll("-", ""), authorPos: $, authorCount: n, citation: d, adjustedCitation: d * s, adjustmentWeight: s, authorshipIndex: a }; Publications[a].push(l), tsvContent += `${l.id}	${l.title}	${l.authorPos}	${l.authorCount}	${l.citation}	${l.adjustedCitation}	${l.adjustmentWeight}
`, selectedList.push([a, i])
    }
} function createButtons() { let t = document.createElement("button"); t.id = "inject-content-button", t.textContent = "Run GScholarLENS", t.textContent.color = "white", t.style.display = "flex", t.style.alignItems = "center", t.style.gap = "5px", t.style.fontSize = "16px", t.style.color = "white", t.style.marginTop = "10px", t.style.marginBottom = "10px", t.style.zIndex = "1000", t.style.borderRadius = "7px", t.style.cursor = "pointer", t.style.boxShadow = "3px 3px 5px rgba(0,0,0,0.6)", t.style.fontFamily = "schibsted-grotesk, sans-serif", t.style.backgroundColor = "rgba(0,0,0,0.8)"; let o = document.createElement("img"); o.src = "icons/icon128.png", o.alt = "GScholarLENS", o.style.height = "20px", o.style.width = "20px", o.style.objectFit = "contain", t.prepend(o); let r = document.getElementById("scholarlens-button"); r.append(t); let e = document.createElement("button"); e.style.display = "none", e.style.fontFamily = "schibsted-grotesk, sans-serif", e.textContent = "Download Publications", e.addEventListener("click", () => { let t = new Blob([tsvContent], { type: "text/tab-separated-values" }), o = URL.createObjectURL(t), r = document.createElement("a"); r.href = o, r.download = "sample_publications.tsv", r.click() }); let a = document.getElementById("download-button"); a.append(e) } function createShIndex() { let t = [0, 0, 0, 0], o = [], r = 0; Publications.forEach((r, e) => { r.sort((t, o) => o.adjustedCitation - t.adjustedCitation), r.forEach((r, a) => { let n = Number(r.adjustedCitation); o.push(n), n >= a + 1 && (t[e] = a + 1) }) }), o.sort((t, o) => o - t), o.forEach((t, o) => { parseInt(t) > r && r++ }), document.getElementById("sh_index").textContent = `Sh-Index:${r.toFixed(0)}`, document.getElementById("h_first").textContent = t[0].toString(), document.getElementById("h_second").textContent = t[1].toString(), document.getElementById("h_other").textContent = t[2].toString(), document.getElementById("h_co").textContent = t[3].toString() } createQScorePosStackedChart(), createAuthorCitationsChart(), createAuthorCitationsDistChart(), createAuthorStackedChart(), createCitationsStackedChart(), generatePublications(), createShIndex(), colorWords(), blinkText("redWord", 500), createButtons();